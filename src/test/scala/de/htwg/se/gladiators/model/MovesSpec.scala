...
class MovesSpec extends AnyWordSpec with Matchers {
    "A Move" when {
        "categorizing moving units" should {
            val enemyPlayer = Player("", 2, 0, 100, Vector())
            val board = createNormalBoard
            "categorize legal moves" in {
                val currentPlayer = Player("", 0, 0, 100, Vector(createGladiator(position = Some(Coordinate(0, 0)), movementPoints = Some(4), moved = Some(false))))
            ...
            "categorize a tile as blocked" in {
                val currentPlayer = Player("", 0, 0, 100, Vector(createGladiator(position = Some(Coordinate(0, 0)), movementPoints = Some(1), moved = Some(false)), createGladiator(position = Some(Coordinate(0, 1)))))
            ...
            "categorize a free tile as no unit at this Tile" in {
                val currentPlayer = Player("", 0, 0, 100, Vector())
            ...
            "categorize a move to palm" in {
                val currentPlayer = Player("", 0, 0, 100, Vector(createGladiator(position = Some(Coordinate(0, 1)), moved = Some(false))))
            ...
            "categorize an illegal move when moving onto the own Base" in {
                val currentPlayer = Player("", 0, 0, 100, Vector(createGladiator(position = Some(Coordinate(2, 2)), movementPoints = Some(1), moved = Some(false))))
            ...
            "categorize a base-attack" in {
                val currentPlayer = Player("", 0, 0, 100, Vector(createGladiator(position = Some(Coordinate(0, 0)), moved = Some(false))))
