...
class MovesSpec extends AnyWordSpec with Matchers {
    "A Move" when {
        "categorizing moving units" should {
            val enemyPlayer = PlayerFactory(enemyBaseLine = 2)
            val board = createNormalBoard
            "categorize legal moves" in {
                val currentPlayer = PlayerFactory(gladiators = Vector(createGladiator(position = Some(Coordinate(0, 0)), movementPoints = Some(4), moved = Some(false))))
            ...
            "categorize illegal moves" in {
                val currentPlayer = PlayerFactory(gladiators = Vector(createGladiator(position = Some(Coordinate(0, 0)), movementPoints = Some(0), moved = Some(false))))
            ...
            "categorize a tile as blocked" in {
                val currentPlayer = PlayerFactory(gladiators = Vector(createGladiator(position = Some(Coordinate(0, 0)), movementPoints = Some(1), moved = Some(false)), createGladiator(position = Some(Coordinate(0, 1)))))
            ...
            "categorize a free tile as no unit at this Tile" in {
                val currentPlayer = PlayerFactory(gladiators = Vector())
            ...
            "categorize a move to palm" in {
                val currentPlayer = PlayerFactory(gladiators = Vector(createGladiator(position = Some(Coordinate(0, 1)), moved = Some(false))))
            ...
            "categorize an illegal move when moving onto the own Base" in {
                val currentPlayer = PlayerFactory(gladiators = Vector(createGladiator(position = Some(Coordinate(2, 2)), movementPoints = Some(1), moved = Some(false))))
            ...
            "categorize a base-attack" in {
                val currentPlayer = PlayerFactory(gladiators = Vector(createGladiator(position = Some(Coordinate(0, 0)), moved = Some(false))))
            ...
            "categorize that the Gladiator already moved" in {
                val currentPlayer = PlayerFactory(gladiators = Vector(createGladiator(position = Some(Coordinate(0, 0)), movementPoints = Some(2))))
            ...
            "categorize mining" in {
                val currentPlayer = PlayerFactory(gladiators = Vector(createGladiator(position = Some(Coordinate(0, 0)), movementPoints = Some(2), moved = Some(false))))
